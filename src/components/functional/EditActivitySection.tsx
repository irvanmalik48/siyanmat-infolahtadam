...
export default function EditActivitySection({ code }: { code: string }) {
  const { data: activity, isLoading: isLoading } = useStaleWhileRevalidate<
    ActivitySubmit & {
      tools: {
        activityId: string;
        toolId: string;
  ...
  return (
    <>
      ...
      {!isLoading && !isLoadingTools ? (
            ...
            <Form className="mt-8 flex w-full flex-col gap-5 rounded-xl border border-neutral-300 p-5">
              ...
              <div className="flex w-full flex-col items-start justify-start gap-1">
                ...
                {Array.from(
                  {
                    length:
                      amountOfTools === 0
                        ? (activity?.tools.length as number)
                        : amountOfTools,
                  },
                  (_, i) => i
                ).map((i) => (
                  <Field
                    ...
                    defaultValue=""
                  >
                    <option value="">Pilih alat {i + 1}</option>
                    {tools?.map((tool) => (
                ...
                <button
                  ...
                  className="w-full self-end rounded-full bg-celtic-800 px-7 py-2 font-semibold text-white transition hover:bg-celtic-700 disabled:brightness-50"
                  onClick={(e) => {
                    e.preventDefault();
                    setAmountOfTools(
                      amountOfTools === 0
              ...
              <motion.div
                ...
                animate={{
                  height:
                    errors.toolUsage && touched.toolUsage ? "110px" : "70px",
                }}
                exit={{
                  height: "70px",
                }}
        ...
        <div className="mt-8 h-[710px] w-full animate-pulse rounded-xl border border-neutral-300 bg-neutral-200 p-5" />
      )}
    </>
  );
}
