...
export default function TambahKegiatan() {
  ...
  const editKegiatanHandler = trpc.editKegiatan.useMutation();
  const getAllPeralatanHandler = trpc.getAllPeralatan.useMutation();
  const diffMenitPemakaianHandler = trpc.diffMenitPemakaian.useMutation();
  const getKegiatanByIdHandler = trpc.getKegiatanById.useMutation();

  const onSubmit = useCallback(async (data: IKegiatan) => {
    try {
      const resOne = await editKegiatanHandler.mutateAsync({
        ...data,
        tglKegiatan: (new Date(data.tglKegiatan)).toISOString(),
        waktuGuna: (parseInt(data.waktuGuna.toString())),
      });
      const resTwo = await diffMenitPemakaianHandler.mutateAsync({
        id: data.idPeralatan,
  ...
  useEffect(() => {
    getKegiatanByIdHandler.mutateAsync({
      id: slug as string,
    }).then((res) => {
      if (res.result === null) return;
      setKegiatan(res.result);
      setDate(new Date(res.result.tglKegiatan).toISOString().split("T")[0]);
    });
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<IKegiatan>({
    defaultValues: {
      ...
      tglKegiatan: kegiatan?.tglKegiatan,
      idPeralatan: kegiatan?.idPeralatan,
      waktuGuna: kegiatan?.waktuGuna,
  ...
  return (
    <DashLayout
      title="Edit Kegiatan"
      description="Page untuk mengedit kegiatan"
    >
      <h1 className="text-4xl font-bold">
        Edit Kegiatan
      </h1>
      <p className="mt-2">
        Silahkan ubah data kegiatan yang ingin diubah di form berikut.
      </p>
      <form className="mt-8" onSubmit={handleSubmit(onSubmit)}>
        <input id="id" type="text" hidden {...register("id", { required: true })} value={kegiatan?.id} />
