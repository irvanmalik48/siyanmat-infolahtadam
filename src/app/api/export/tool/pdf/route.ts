...
<body style="font-family: sans-serif; font-size: 12px; min-height: 50vh; display: flex; flex-direction: column;">
    ...
    <p style="margin: 0;">
        <span style="width: 100px; display: inline-block;">Nama Alat</span>: ${tool?.name}
    </p>
    <p style="margin: 0;">
        <span style="width: 100px; display: inline-block;">Merek</span>: ${tool?.brand}
    </p>
    <p style="margin: 0;">
        <span style="width: 100px; display: inline-block;">Maks Usia</span>: ${tool?.maxHourUsage} jam
    </p>
    <p style="margin: 0;">
        <span style="width: 100px; display: inline-block;">Sisa Usia</span>: ${tool?.maxHourUsage - tool?.activities.reduce((acc, curr) => acc + curr.activity.toolUsage, 0)} jam
    </p>
    <p style="margin: 0;">
        <span style="width: 100px; display: inline-block;">Kondisi</span>: ${tool?.condition}
    </p>
    ...
    <table style="border: 1px black solid; border-collapse: collapse; width: 100%; margin-top: 6px; margin-bottom: auto;">
        ...
        <tbody>
            ${tool?.activities.map((activity, index) => /* html */`
                <tr>
                    <td style="border: 1px black solid; text-align: center;">${index + 1}</td>
                    <td style="border: 1px black solid; text-align: center; text-align: left; padding-left: 5px; padding-right: 5px;">${activity.activity.name}</td>
                    <td style="border: 1px black solid; text-align: center;">${activity.activity.date.toLocaleDateString("id-ID", {
    day: "numeric",
    month: "numeric",
    year: "numeric",
  })}</td>
                    <td style="border: 1px black solid; text-align: center;">${activity.activity.toolUsage} jam</td>
                    <td style="border: 1px black solid; text-align: center;">${activity.activity.operatorName}</td>
                </tr>
              `).join("")
    }
        </tbody>
